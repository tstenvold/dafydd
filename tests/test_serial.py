...
class TestSerialDiscoveryConfiguration:
    ...
    def test_port_filter_accepted(self):
        """Test that port_filter parameter is accepted."""
        discovery = dafydd.SerialDiscovery(
            b"*IDN?\r\n",
            [9600],
            port_filter="/dev/ttyUSB",
        )
        assert discovery is not None

    def test_port_filter_with_windows_style(self):
        """Test that port_filter works with Windows-style port names."""
        discovery = dafydd.SerialDiscovery(b"", [9600], port_filter="COM")
        assert discovery is not None
