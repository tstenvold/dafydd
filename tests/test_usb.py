...
class TestUsbDiscoveryEdgeCases:
    ...
    def test_device_class_filter_accepted(self):
        """Test that device_class filter parameter is accepted."""
        disc = dafydd.UsbDiscovery(device_class=0x03)  # HID class
        assert disc is not None

    def test_device_class_filter_no_match_returns_list(self):
        """Test that device_class filter with no match returns empty list."""
        results = dafydd.UsbDiscovery(device_class=0xFE).discover()
        assert isinstance(results, list)

    def test_device_class_combined_with_vid(self):
        """Test device_class combined with VID filter."""
        disc = dafydd.UsbDiscovery(vid=0x1234, device_class=0x03)
        assert disc is not None
